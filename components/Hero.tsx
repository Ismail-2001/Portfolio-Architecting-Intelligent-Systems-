...
const Hero: React.FC = () => {
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden px-6 pt-24 lg:pt-0" aria-labelledby="hero-title">
      <style>{`
        @keyframes noise {
        ...
        .animate-noise { animation: noise 4s steps(8) infinite; }
      ...
      <div className="absolute top-[-20%] left-[-20%] w-[140%] h-[140%] bg-[url('https://grainy-gradients.vercel.app/noise.svg')] opacity-[0.07] pointer-events-none animate-noise mix-blend-overlay" aria-hidden="true" />
      ...
      <div className="container max-w-7xl mx-auto grid lg:grid-cols-2 gap-12 lg:gap-8 items-center relative z-10">
        <div className="flex flex-col items-center lg:items-start text-center lg:text-left space-y-8">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-cyan-500/30 bg-cyan-500/5 backdrop-blur-sm">
                <CircuitBoard className="w-4 h-4 text-cyan-400" aria-hidden="true" />
                <span className="text-xs font-medium text-cyan-300 tracking-wide uppercase">System Architecture Online</span>
            </div>

            <h1 id="hero-title" className="text-4xl sm:text-5xl lg:text-7xl font-bold tracking-tight text-white leading-[1.1]">
            ...
            <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
              <button 
                className="group relative px-8 py-4 bg-white text-slate-950 font-semibold text-lg rounded-full transition-all hover:bg-cyan-50 hover:shadow-[0_0_30px_rgba(34,211,238,0.3)] hover:-translate-y-1 flex items-center justify-center gap-2"
                aria-label="Explore engineering portfolio projects"
              >
                {HERO_CONTENT.cta}
                <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
              </button>
              <button 
                className="px-8 py-4 border border-white/10 bg-white/5 text-white font-semibold text-lg rounded-full transition-all hover:bg-white/10 hover:border-white/20 flex items-center justify-center"
                aria-label="View technical project repository"
              >
                View Stack
              </button>
            </div>
        ...
        <div className="relative w-full max-w-md mx-auto lg:max-w-full lg:ml-auto perspective-1000" aria-hidden="true">
            <div className="relative group">
                ...
                <div className="relative rounded-[2rem] overflow-hidden border border-white/10 bg-slate-900/80 backdrop-blur-sm">
                    <img 
                      ...
                      alt="Alex Mercer - Senior AI Engineer" 
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-6 space-y-3">
                        <div className="flex items-center gap-3">
                             ...
                             <span className="text-xs text-cyan-400 font-mono">SYS_INTEGRITY_99.9%</span>
                        </div>
                        <div className="flex items-center justify-between glass-panel p-3 rounded-xl border-white/10 bg-slate-900/60 backdrop-blur-md">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <div className="text-xs text-slate-400">Next Gen Inference</div>
                                    <div className="text-sm font-bold text-white">Gemini 3 Pro Hybrid</div>
                                </div>
                            </div>
                        </div>
                    </div>
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 animate-bounce text-slate-600 hidden lg:block" aria-hidden="true">
